 ...
 */
const props = defineProps<{ entry: PresentedRow; metric: 'metric1' | 'metric2' }>()
...
const target = computed<EntitySchema | null>(() => {
  ...
  const key = entity.drills?.[props.metric]
  return shell.entities.value.find((candidate) => candidate.key === key) ?? null
})

const text = computed(() => props.entry[props.metric])
...
<template>
  <button
    ...
    type="button"
    class="dc-drill"
    :title="`${entry.labels[metric]} of ${entry.row.primary} — show the ${target.label.toLowerCase()}`"
    @click="drill"
  >
    <slot>{{ text }}</slot>
  </button>
